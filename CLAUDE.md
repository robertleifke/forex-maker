...
**Sell amount must come from the actual buy result.** After the buy leg completes, the sell leg must use `buy_trade.amount` (the cNGN actually received), not any pre-buy estimate. Preflights may use an estimate; live execution must not. Using a stale estimate causes the sell to attempt an amount the wallet doesn't hold if the buy slips, or leave cNGN stranded if the buy over-delivers.
