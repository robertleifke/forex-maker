...
**CEX and DEX accounts are completely separate wallets, sometimes on different chains.** The Quidax CEX account and the DEX trade accounts (`UNI_BASE_TRADE`, `UNI_BSC_TRADE`) hold independent balances. A CEX buy delivers cNGN to the CEX account; a DEX sell draws cNGN from the DEX trade wallet on-chain. These never share a balance. When reviewing any path that crosses a CEX and a DEX leg, do not assume that what the CEX delivers is available to the DEX, or vice versa. Inventory caps on each leg are enforced independently.
