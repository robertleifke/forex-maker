...
**Fill data settles asynchronously.** Quidax fills market orders immediately, but the creation response often returns `executed_volume`/`avg_price` as zero. `place_market_order` then polls the order (`fetch_order_by_id`, five attempts with short backoff, ~8 s budget) until both fields are positive. Only when polling is exhausted does it return zeros — with a warning alert — which routes the executor to its signal-price estimate as the genuine last resort. Once an order is accepted, nothing may re-enter the placement retry loop: a malformed fill field degrades to the poll path rather than raising, because a resubmission would double-trade. Pinned in `tests/test_quidax_adapter.py` (the `market_order_*` fill-poll tests).
