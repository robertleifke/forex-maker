...
1. Accept that this isn't a real opportunity — a 59 bps gross spread against a 100+ bps PancakeSwap swap fee is genuinely unprofitable.

2. Lower `min_spread_bps` only if you have evidence the pool fees are actually lower (the engine will show you actual fees in logs once it fetches them from chain).

3. For a DEX↔CEX arb (e.g. Aerodrome ↔ Quidax), the fee structure is entirely different: one DEX swap fee + one CEX taker fee — typically 30–100 bps total depending on the DEX pool tier.

For cross-chain DEX↔DEX arb (Aerodrome ↔ PancakeSwap), the PancakeSwap pool's 1% fee alone means you need well over **150 bps gross spread** to profit.
